...
## Amendment A2 — 2026-07-06: scrollable week-scale timeline

The timeline is not compressed to one screen. Fixed scale: **one ISO week ≈ one viewport**; full span ≈ 13 screens wide, scrolled left→right. Rendering is virtualized — the canvas stays viewport-sized and pans (world-space translate + culling); the equity SVG pans via a group transform. Lane captions stay pinned. The hero stage is a native horizontal scroller; the narrative layer maps vertical scroll to the pan (week chapters as beats along the way); the explorer uses the same scroller pattern with week-rail jumps. `prefers-reduced-motion` / mobile keep a fit-to-screen overview instead of panning.
