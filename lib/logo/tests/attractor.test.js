...
// Regression coordinates reflect Float32 storage truncation (~7 decimal digits).
// Tolerance 1e-5 gives ~2 digits of headroom.
function near(a, b, tol = 1e-5) { return Math.abs(a - b) <= tol; }

assert('regression: point #0 (element 0)',
  near(pts[0], 0.989591) && near(pts[1], -0.450130));
assert('regression: point #1000 (element 2000)',
  near(pts[2000], -0.257796) && near(pts[2001], 0.904991));
assert('regression: point #40000 (element 80000)',
  near(pts[80000], 0.363975) && near(pts[80001], 0.288321));
assert('regression: point #79999 (element 159998)',
