  ...
  function draw({ revealDay = Infinity, t = 0, highlightId = null, filter = null, panX = 0 } = {}) {
    ...
    for (const market of ['eu', 'us']) {
      for (const m of markersByMarket[market]) {
        ...
        ctx.fillRect(m.x - 7, yTop - 4, 14, m.h + 8);

        if (m.filled) {
          ctx.fillStyle = color;
          ctx.fillRect(m.x - 2.5, yTop, 5, m.h);
        } else {
          ctx.strokeStyle = color;
          ctx.lineWidth = 1.5;
          ctx.strokeRect(m.x - 2.5, yTop, 5, m.h);
        }
        ctx.fillStyle = color;                              // cap tick, candle feel
        const capY = m.dir === -1 ? yTop : yTop + m.h;
        ctx.fillRect(m.x - 4.5, capY - 0.75, 9, 1.5);

        if (m.id === highlightId) {
          ctx.strokeStyle = WIN(0.9);
          ctx.lineWidth = 1;
          ctx.strokeRect(m.x - 7.5, yTop - 5, 15, m.h + 10);
        }
      }
    }
