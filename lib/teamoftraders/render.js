...
export function createRenderer(canvas, { markersByMarket, bands, lanes, annotations = [] }) {
  ...
  function markerColor(m, alphaScale = 1) {
    if (!m.filled) return AMBER(0.5 * alphaScale);      // buys: neutral cost bars
  ...
  function draw({ revealDay = Infinity, t = 0, highlightId = null, filter = null, panX = 0 } = {}) {
    ...
    for (const market of ['eu', 'us']) {
      for (const m of markersByMarket[market]) {
        ...
        const yTop = (m.dir === -1 ? m.y - m.h : m.y) + drift;
        const color = markerColor(m, alphaScale);

        ctx.fillStyle = markerColor(m, 0.12 * alphaScale); // glow sheath
        ctx.fillRect(m.x - 4.5, yTop - 3, 9, m.h + 6);

        if (m.filled) {
          ctx.fillStyle = color;
          ctx.fillRect(m.x - 1.5, yTop, 3, m.h);
        } else {
          ctx.strokeStyle = color;
          ctx.lineWidth = 1;
          ctx.strokeRect(m.x - 1.5, yTop, 3, m.h);
        }
        ctx.fillStyle = color;                              // cap tick, candle feel
        const capY = m.dir === -1 ? yTop : yTop + m.h;
        ctx.fillRect(m.x - 3.5, capY - 0.5, 7, 1);

        if (m.id === highlightId) {
          ctx.strokeStyle = WIN(0.9);
          ctx.lineWidth = 1;
          ctx.strokeRect(m.x - 5.5, yTop - 4, 11, m.h + 8);
        }
      }
    }
