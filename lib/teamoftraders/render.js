...
import { BAR_W } from './strata.js';

const WIN = a => `rgba(240, 200, 130, ${a})`;
const AMBER = a => `rgba(220, 180, 100, ${a})`;

const HALF = BAR_W / 2;        // body half-width
const CAP = HALF + 2;          // end-cap tick overhangs the body by 2px
const SHEATH = HALF + 4;       // glow sheath half-width
  ...
  function draw({ revealDay = Infinity, t = 0, highlightId = null, filter = null, panX = 0 } = {}) {
    ...
    for (const market of ['eu', 'us']) {
      for (const m of markersByMarket[market]) {
        ...
        ctx.fillRect(m.x - SHEATH, yTop - 5, SHEATH * 2, m.h + 10);

        if (m.filled) {
          ctx.fillStyle = color;
          ctx.fillRect(m.x - HALF, yTop, BAR_W, m.h);
        } else {
          ctx.strokeStyle = color;
          ctx.lineWidth = 2;
          ctx.strokeRect(m.x - HALF, yTop, BAR_W, m.h);
        }
        ctx.fillStyle = color;                              // cap tick at the outer end
        const capY = m.dir === -1 ? yTop : yTop + m.h;
        ctx.fillRect(m.x - CAP, capY - 1, CAP * 2, 2);

        // Ticker — centred on the bar, one row (the bar is as wide as its name).
        ctx.font = '300 11px "DM Mono", monospace';
        ctx.textAlign = 'center';
        ctx.fillStyle = markerColor(m, 0.7 * alphaScale);
        ctx.fillText(m.ticker, m.x, m.dir === -1 ? yTop - 9 : yTop + m.h + 16);

        if (m.id === highlightId) {
          ctx.strokeStyle = WIN(0.9);
          ctx.lineWidth = 1;
          ctx.strokeRect(m.x - HALF - 1, yTop - 6, BAR_W + 2, m.h + 12);
        }
      }
    }
