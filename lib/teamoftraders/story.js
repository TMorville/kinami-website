...
// at their week's world-x, lighting up as they pan into view. Equity lines
// draw on with pan progress; markers reveal just ahead of the pan edge.
...
export function initStory(index, { cardsEl, equityPaths, lanes }) {
  ...
  const cards = stops.map(stop => {
    ...
    let worldX = Math.max(X(stop.day), lastRight + HALF + 24);
    worldX = Math.min(worldX, lanes.x0 + lanes.width - HALF);
  ...
  // Draw-on: dash each solid equity path to its own length; offset follows pan.
  const dash = [];
  ...
  /** Per-frame: pan the cards layer, light visible cards, draw lines on.
  ...
  function update(panX, viewportW) {
    ...
    // Denominator is slightly short of full width so progress reaches 1 while
    // the scroller can still travel there — otherwise the line never finishes.
    const edgeX = panX + viewportW * 0.85;
    const progress = Math.min(Math.max((edgeX - lanes.x0) / (lanes.width * 0.98), 0), 1);
    for (const { path, len } of dash) {
