...
export const SLOT_SPREAD = 44;   // side-by-side distance between same-day bars
 ...
 *  grow UP, losses hang DOWN. Same-day trades sit side by side, spread
 *  SLOT_SPREAD apart so their ticker labels have room; bars clamp to their
 *  lane so a low or high curve never pushes them off-screen. */
export function layoutMarket(index, market,
  ...
  return trades.map(t => {
    ...
    // Clamp into the lane, keeping ~28px for the two label rows.
    const room = (dir === -1 ? y - yMin : yMax - y) - 28;
