...
export function layoutMarket(index, market, { x0, width, yScale, anchors }) {
  ...
  // Same-day trades fan out horizontally inside their day cell instead of
  // stacking on one x; `slot` also staggers the ticker labels.
  ...
  return trades.map(t => {
    ...
    const spread = (slot - (dayCounts.get(day) - 1) / 2) * 16;
