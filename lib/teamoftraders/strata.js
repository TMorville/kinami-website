// Pure timeline layout: time flows left→right; trades become markers sitting
// on their market's equity path; ISO weeks tile the x axis as stripes.
// Deterministic (hash01 jitter) so narrative and explorer render identically.
import { hash01 } from './data.js';

export const MARK_MIN_R = 2;
export const MARK_MAX_R = 10;

/** Shared x-scale: fractional day index → x. */
export function dayX(index, x0, width) {
  const span = Math.max(index.dayList.length - 1, 1);
  return day => x0 + (day / span) * width;
}
...
/** Lay out one market's trades as markers on its equity path. */
export function layoutMarket(index, market, { x0, width, yScale, anchors }) {
  ...
  return trades.map(t => {
    ...
    const jitter = (hash01(t.id) - 0.5) * 18; // bloom around the curve, ±9px
    return {
      id: t.id,
      day,
      x: X(day),
      y: yScale(equityAtDay(day, anchors)) + jitter,
      r: MARK_MIN_R + Math.sqrt(t.notional / maxNotional) * (MARK_MAX_R - MARK_MIN_R),
      ring: t.action === 'sell', // sells carve — rendered as rings, buys as dots
...
/** Contiguous ISO-week stripes across the shared day axis. */
export function weekBands(index, x0, width) {
  ...
  index.dayList.forEach((d, i) => {
    ...
    if (!current || current.week !== week) {
      current = { week, x0: X(i), x1: X(i + 1) };
      bands.push(current);
    } else {
      current.x1 = X(i + 1);
    }
  });
  if (bands.length) bands[bands.length - 1].x1 = X(index.dayList.length - 1);
  return bands;
}
