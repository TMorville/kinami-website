...
import {
  ...
  dayX, BAR_MIN_H, BAR_MAX_H, STACK_GAP,
} from '../strata.js';
...
assert('buys stack down', markers[0].dir === 1 && markers[0].yTop === markers[0].y);
...
// Same-day trades stack vertically on one x, separated by STACK_GAP.
const rawStack = JSON.parse(JSON.stringify(raw));
rawStack.markets.eu.trades.push({ id: 'eu-0003', date: '2026-04-08T15:00:00', ticker: 'SAP',
  action: 'buy', notional: 500, is_win: null, week_id: '2026-W15' });
const idxStack = indexShowcase(rawStack);
const stacked = layoutMarket(idxStack, 'eu', { x0: 50, width: 100, yScale, anchors });
assert('same-day segments share one x', stacked[0].x === stacked[2].x);
assert('second same-day segment stacks below the first',
  stacked[2].yTop === stacked[0].yTop + stacked[0].h + STACK_GAP);
...
assert('winning sells stack up', winMarkers[1].dir === -1 && winMarkers[1].yTop === winMarkers[1].y - winMarkers[1].h);
