...
// A wide pre-record gap eases (smoothstep) from start capital into the first
// snapshot — a monotone bridge, not a single dead-straight ramp.
const rawGap = {
  markets: {
    eu: {
      starting_capital: 10000,
      trades: [
        { id: 'e1', date: '2026-04-08T10:00:00', ticker: 'DTE', action: 'buy', notional: 100, is_win: null, week_id: '2026-W15' },
        { id: 'e2', date: '2026-04-20T10:00:00', ticker: 'SAP', action: 'buy', notional: 100, is_win: null, week_id: '2026-W17' },
      ],
      equity: [{ date: '2026-04-30', equity: 9000, benchmark_equity: 9000 }],
      weeks: [],
    },
    us: { starting_capital: 10000, trades: [], equity: [], weeks: [] },
  },
...
const gapAnchors = buildAnchors(indexShowcase(rawGap), 'eu');
// dayList = ['2026-04-08'(0), '2026-04-20'(1), '2026-04-30'(2)]; snapshot at day 2.
assert('eased bridge keeps the honest start value',
  gapAnchors[0].day === 0 && gapAnchors[0].value === 10000);
assert('eased bridge inserts an intermediate anchor',
  gapAnchors.length === 3 && gapAnchors[1].day === 1);
assert('eased bridge is monotone toward the snapshot',
  gapAnchors[0].value > gapAnchors[1].value && gapAnchors[1].value > gapAnchors[2].value);
assert('eased bridge ends on the real snapshot',
  gapAnchors[2].day === 2 && gapAnchors[2].value === 9000);
...
assert('down bars clamp to lane room', clamped[1].h <= 40 - 18 + 1e-9);
