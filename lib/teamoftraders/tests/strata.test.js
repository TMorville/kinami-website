...
// Same-day trades fan out instead of stacking on one x.
...
assert('same-day bars fan out', Math.abs(stacked[0].x - stacked[2].x) === 16);
assert('slots assigned in order', stacked[0].slot === 0 && stacked[2].slot === 1);
