...
import { linePath, smoothPath, lanePaths } from '../equity.js';
...
assert('smoothPath empty → empty string', smoothPath([]) === '');
assert('smoothPath single point → M only', smoothPath([{ x: 1, y: 2 }]) === 'M1.0 2.0');
const twoPt = smoothPath([{ x: 0, y: 0 }, { x: 10, y: 0 }]);
assert('smoothPath two points → cubic ending on the last point',
  twoPt.startsWith('M0.0 0.0 C') && twoPt.endsWith('10.0 0.0'));
...
const { equity, benchmark } = lanePaths(index, 'eu', geom);
assert('equity runs continuously from the synthetic start anchor',
  equity.startsWith(`M16.7 ${yScale(10000).toFixed(1)}`));
assert('equity is a smooth bezier curve', equity.includes(' C'));
assert('equity smoothly passes through the first snapshot',
  equity.includes(`50.0 ${yScale(9000).toFixed(1)}`));
assert('equity ends at the last snapshot',
  equity.endsWith(`83.3 ${yScale(9500).toFixed(1)}`));
assert('benchmark path uses benchmark values',
  benchmark.endsWith(`L83.3 ${yScale(9100).toFixed(1)}`));

// No pre-record gap → equity simply starts at the first snapshot.
...
assert('no-gap market starts at its first snapshot', paths2.equity.startsWith('M25.0'));
...
assert('empty market → empty paths', pathsUs.equity === '' && pathsUs.benchmark === '');
