...
/**
 * Pure cell math: cell collapsing, direction statistics, recency input.
 * No map library, no DOM, no timers.
 *
 * Every observation in the snapshot renders; there is no time-range control
 * (removed 2026-09-02 with the two-tone recency simplification, along with
 * the window machinery it required).
 */
 ...
 *
 * `age_h` is measured against the snapshot's generated_at, never the client
 * clock, so the two-tone recency boundary in layers.js is a statement about
 * the data. In the stale state recency freezes with the snapshot; the
 * "updated X ago" line carries the snapshot's own age, and the freshness
 * cliff bounds the drift.
 */
export function cellsToGeoJSON(cells, generatedAtIso) {
  return {
    ...
    features: cells.map((cell) => {
      ...
      return {
        ...
        properties: {
          age_h: ageHours(cell.newestHour, generatedAtIso),
          count: cell.count,
          mark: direction.mark,
